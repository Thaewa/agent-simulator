...
class instanceGenerator:
    ...
    :param number_of_cells: Upper bound on the number of grid cells used to set nest radius.
    ...
    **Derived attributes**
    ...
    :ivar number_of_cells: See parameter of the same name.
    ...
    def generateLarvae(self, x: int, y: int) -> "Larvae":
        ...
        agent_id = "L" + str(np.round(x,2)) + str(np.round(y,2))
        # Randomly choose a hunger multiplier for the larvae
        random_hunger_multiplier = np.random.choice([1.0, self.larvae_dictionary['hunger_rate_multiplier_l2'],self.larvae_dictionary['hunger_rate_multiplier_l2']], 1)[0]
    ...
    def generateGrid(self, radius: int) -> np.ndarray:
        ...
        x = np.arange(-radius-1, radius + self.grid_buffer+0.5, 0.5)
        y = np.arange(-radius-1, radius + self.grid_buffer+0.5, 0.5)
    ...
    def addWasps(
        ...
        for row in chosen_nest_indices_feeders:
            wasp = self.generateWasp(grid[row, 0], grid[row, 1], outer_nest_radius=radius_nest)
    ...
    def generateWasp(self, x: int, y: int, outer_nest_radius: int = None) -> "Wasp":
        """
        ...
        wasp = Wasp(
                agent_id="W" + str(np.round(x,2)) + str(np.round(y,2)),
        ...
        # Assign pathfinding mode directly from simulator_dictionary
        if hasattr(self, "simulator_dictionary") and "pathfinding_mode" in self.simulator_dictionary:
            wasp.path_finding = self.simulator_dictionary["pathfinding_mode"]
        else:
            wasp.path_finding = "unknown"
        return wasp
    ...
    def updateSimulator(self,simulator):
        ...
        simulator.potential_feeder_to_forager = simulator.forager_ratio + self.simulator_dictionary['potential_feeder_to_forager_buffer']
        return simulator
    ...
    def generateSimulator(self) -> "Simulator":
        ...
        # Define the number of wasps as the number of cells
        total_wasps = number_of_cells
        # Define number of larvae
        total_larvae = math.ceil(number_of_cells*self.nest_fill_percentage)
        # Define number of inner larvae
        total_inner_larvae = math.floor(total_larvae*self.proportion_inner_larvae)
        # Define number of outer larvae
        total_outer_larvae = total_larvae - total_inner_larvae
        ...
        radius_nest = math.ceil((number_of_cells/np.pi) ** (1 / 2))
        # Divide the nest in inner and outer nest where each of them simply uses half of the nest radius
        radius_inner_nest = math.ceil(((number_of_cells*self.proportion_inner_larvae)/np.pi) ** (1 / 2))
        ...
        chosen_nest_indices_larvaes = np.random.choice(
            outer_nest_indices, total_outer_larvae, replace=False
        )
        # Filter indices from the grid that are in the forage ring
        forage_indices = np.where(outside_circle(grid[:, 0], grid[:, 1], radius_nest + self.forage_distance))[0]
        ...
        total_foragers = max(1, math.floor(total_wasps * simulator.forager_ratio))
        ...
        chosen_nest_indices_foragers = np.random.choice(outer_nest_indices, total_foragers, replace=False)
        ...
        simulator = self.addWasps(
            chosen_nest_indices_feeders, chosen_nest_indices_foragers, grid, simulator,radius_nest
        )
        return simulator
...
class Simulator:
    ...
    :param number_of_bouts: Number of feeding bouts to simulate per simulation run.
    :type number_of_bouts: int
    ...
    **Attributes initialized in ``__init__``**
    ...
    :ivar number_of_bouts: Number of feeding bouts to simulate per simulation run.
    :vartype number_of_bouts: int
    """

    def __init__(self, forager_ratio: float=0.10, max_role_changes: int=2, role_changes_frequency: int=100,number_of_bouts:int=32) -> None:
        ...
        # Initialize DataLogger (with mode tag)
        # Delay logger creation — will be created later in main.py
        self.logger = None
        self.active_bouts = {}
        self.recent_feeding_events = []
        self.finished_bouts: int = 0

    def stratify_active_bouts(self):
        
        forager_to_feeder_bouts_foragers = [bout.feeder for bout in self.recent_feeding_events]
        forager_to_feeder_bouts_feeders = [bout.receiver for bout in self.recent_feeding_events]
        new_active_bouts = defaultdict(list)
        for k, v in zip(forager_to_feeder_bouts_foragers, forager_to_feeder_bouts_feeders):
            new_active_bouts[k].append(v)
        for k in new_active_bouts.keys():
            if k not in self.active_bouts:
                self.active_bouts[k] = new_active_bouts[k]
            else:
                self.active_bouts[k].extend(new_active_bouts[k])
        self.recent_feeding_events = []

    def count_bouts(self):
        if self.active_bouts != {}:
            leading_wasps = list(self.active_bouts.keys())
            for wasp in leading_wasps:
                following_wasp = self.active_bouts[wasp]
                following_wasp_hunger = [wasp.food for wasp in following_wasp]
                following_wasp_hunger = [food<1.5 for food in following_wasp_hunger]
                following_wasp_hunger,total_following_wasp = sum(following_wasp_hunger),len(following_wasp_hunger)
                if (np.round(following_wasp_hunger/total_following_wasp,2)>0.5 or following_wasp_hunger==1) and wasp.food<1.1:
                    del self.active_bouts[wasp]
                    if len(self.active_bouts) == 0:
                        self.finished_bouts += 1
                        self.active_bouts = {}
                        self.recent_feeding_events = []
                        print("finished bout",self.finished_bouts)
                        return                    
    ...
    def runSimulation(self, t: int) -> Dict[str, Dict]:
        ...
        # Log initial state at timestamp = 0
        self.currentTime = 0
        if self.logger:
            # Log all wasps (agents)
            for agent in self.agents:
                self.logger.log_agent(
                    ...
                    nest_layer=(1 if isinstance(agent, Larvae)
            ...
            # Log larvae state
            for larva in [a for a in self.agents if isinstance(a, Larvae)]:
        ...
        while i < t:
            ...
            # Log nest + larvae summary BEFORE incrementing time
            self.logger.log_nest(
                timestamp=self.currentTime,
                total_foragers=len([a for a in self.agents if isinstance(a, Wasp) and a.role == WaspRole.FORAGER]),
                total_feeders=len([a for a in self.agents if isinstance(a, Wasp) and a.role == WaspRole.FEEDER]),
            ...
            # Added here — move self.currentTime increment after all logging
            self.currentTime += 1

            if (i + 1) % 10 == 0:
                num_foragers = sum(isinstance(a, Wasp) and a.role == WaspRole.FORAGER for a in self.agents)
                num_feeders = sum(isinstance(a, Wasp) and a.role == WaspRole.FEEDER for a in self.agents)
                print(f"[Step {i+1:04d}] Time={self.currentTime:04d} | Foragers={num_foragers} | Feeders={num_feeders}")

            i += 1
            self.clearGradients()
            self.stratify_active_bouts()
            self.count_bouts()
            # if self.finished_bouts >=self.number_of_bouts:
            #     break
        # ======================================================================
        ...
        # Separate feed count tracking
        feed_counts_wasp = [w.feed_count_wasp for w in wasps if hasattr(w, "feed_count_wasp")] if wasps else []
        feed_counts_larvae = [w.feed_count_larvae for w in wasps if hasattr(w, "feed_count_larvae")] if wasps else []

        mean_feed_freq_wasp = np.mean(feed_counts_wasp) if feed_counts_wasp else 0
        std_feed_freq_wasp  = np.std(feed_counts_wasp) if feed_counts_wasp else 0
        mean_feed_freq_larvae = np.mean(feed_counts_larvae) if feed_counts_larvae else 0
        std_feed_freq_larvae  = np.std(feed_counts_larvae) if feed_counts_larvae else 0

        num_wasps = len(wasps)
        num_larvae = len(larvae)
        wasp_to_larvae_ratio = num_wasps / num_larvae if num_larvae > 0 else 0

        # Safe retrieval of mode from config
        pathfinding_mode = "unknown"
        if hasattr(self, "config") and isinstance(self.config, dict):
            pathfinding_mode = self.config.get("simulator", {}).get("pathfinding_mode", "unknown")

        results = {
            "simulation_id": datetime.now().strftime("%Y%m%d_%H%M%S"),
            "pathfinding_mode": pathfinding_mode,
            "mean_hunger_larvae": np.mean([a.hunger for a in larvae]) if larvae else 0,
            "max_hunger_larvae": np.max([a.hunger for a in larvae]) if larvae else 0,
            "min_hunger_larvae": np.min([a.hunger for a in larvae]) if larvae else 0,
            "mean_distance_per_feed_larvae": self._calc_distance_per_feed_larvae(),
            "mean_distance_per_feed_wasp": self._calc_distance_per_feed_wasp(),
            "feeding_efficiency": self._calc_feeding_efficiency(),
            
            # New separate feed stats
            "mean_feed_freq_wasp": mean_feed_freq_wasp,
            "std_feed_freq_wasp": std_feed_freq_wasp,
            "mean_feed_freq_larvae": mean_feed_freq_larvae,
            "std_feed_freq_larvae": std_feed_freq_larvae,
        ...
        # --- Additional Feeder metrics ---
        wasps = [a for a in self.agents if a.type == AgentType.WASP]
        feeders = [w for w in wasps if hasattr(w, "role") and str(w.role).endswith("FEEDER")]

        results.update({
            "sum_distance_traveled_feeders": sum(w.distance_traveled for w in feeders),
            "sum_feed_count_larvae": sum(w.feed_count_larvae for w in feeders),
            "avg_distance_traveled_feeders": np.mean([w.distance_traveled for w in feeders]) if feeders else 0,
            "avg_feed_count_larvae": np.mean([w.feed_count_larvae for w in feeders]) if feeders else 0,
            "efficiency_walk_feed_ratio": (
                sum(w.feed_count_larvae for w in feeders) / max(sum(w.distance_traveled for w in feeders), 1e-9)
            )
        })

        results.update({
            "sum_step_count_feeders": sum(getattr(w, "step_count", 0) for w in feeders),
            "avg_step_count_feeders": np.mean([getattr(w, "step_count", 0) for w in feeders]) if feeders else 0,
            "efficiency_step_feed_ratio": (
                sum(w.feed_count_larvae for w in feeders) / max(sum(getattr(w, "step_count", 0) for w in feeders), 1e-9)
            )
        })

        # safer: use config attached by main.py
        self.logger.log_aggregate(results, getattr(self, "config", {}))

        larvae_hunger_values = [a.hunger for a in self.agents if a.__class__.__name__ == "Larvae"]
        print(f"Larvae hunger min={np.min(larvae_hunger_values):.2f}, "
              f"mean={np.mean(larvae_hunger_values):.2f}, "
              f"max={np.max(larvae_hunger_values):.2f}")

		# Return structured report for post-processing and plotting
    ...
    # ----------------------------------------------------------------------
    def _calc_distance_per_feed_larvae(self):
        distances = []
        for wasp in [a for a in self.agents if a.__class__.__name__ == "Wasp"]:
            count = getattr(wasp, "feed_count_larvae", 0)
            if count > 0 and hasattr(wasp, "distance_traveled"):
                distances.append(wasp.distance_traveled / count)
        return np.mean(distances) if distances else 0

    def _calc_distance_per_feed_wasp(self):
        distances = []
        for wasp in [a for a in self.agents if a.__class__.__name__ == "Wasp"]:
            count = getattr(wasp, "feed_count_wasp", 0)
            if count > 0 and hasattr(wasp, "distance_traveled"):
                distances.append(wasp.distance_traveled / count)
        return np.mean(distances) if distances else 0
    ...
    def _calc_feeding_efficiency(self):
        ...
        fed = sum(1 for a in larvae if getattr(a, "hunger", 0) < 1.5)
    ...
    def stepAgent(self, agent: Wasp, wasps: List[Wasp], larvaes_position: List[List[int]]) -> None:
        ...
        if agent.role == WaspRole.FORAGER:
            ...
            feeding_events = agent.step(t=self.currentTime, agents=self.agents, forage=self.forage)
            self.recent_feeding_events.extend(feeding_events)
    ...
    def updateRoles(
        ...
        if t % self.role_changes_frequency == 0 and count_roles <= self.max_role_changes:
            ...
            if (
                ...
                and ((total_foragers + 1) / total_wasp) < (self.potential_feeder_to_forager)
            ):
